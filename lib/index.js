...
internals.implementation = function (server, options) {
    ...
    var basicChallenge = settings.disableBasicChallenge ? null : 'Basic';
    ...
    var scheme = {
        authenticate: function (request, reply) {
            ...
            if (!authorization) {
                return reply(Boom.unauthorized(null, basicChallenge));
            }
            ...
            if (!username && !settings.allowEmptyUsername) {
                return reply(Boom.unauthorized('HTTP authentication header missing username', basicChallenge));
            }
            ...
            settings.validateFunc(username, password, function (err, isValid, credentials) {
                ...
                if (!isValid) {
                    return reply(Boom.unauthorized('Bad username or password', basicChallenge), null, { credentials: credentials });
                }
